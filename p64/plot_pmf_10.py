...
#tnr=matplotlib.font_manager.FontProperties(family='Times New Roman')
'''
# SUBPLOT 1
subplot(311)
...
#plt.xlabel('end-to-end distance ($\AA$)',fontproperties=fpropxxl)
#plt.ylabel('PMF (kcal/mol)',fontproperties=fpropxxl)
plt.yticks((0,5,10,15,20,25),fontproperties=fpropm,fontname='Times New Roman')
plt.xticks((15,20,25,30),fontproperties=fpropm,fontname='Times New Roman')
plt.ylim([-3,26])             # manually define
plt.xlim([12.0,34.0])           # manually define
def plot_vel(vel):
    v = int(vel)
    #plot_pmf(**{'method':'asmd','sample_size':800,'velocity':v, \
            #'solvent':'imp','color_style':'r--','label':'ASMD 800 tps'})
    #plot_pmf(**{'method':'asmd','sample_size':100,'velocity':x, \
     #       'solvent':'exp','color_style':'y.','label':'ASMD 100e at 1'})
    #plot_pmf(**{'method':'asmd','sample_size':400,'velocity':v, \
            #'solvent':'imp','color_style':'b-.','label':'ASMD 400 tps'})
    plot_pmf(**{'method':'asmd','sample_size':100,'velocity':v, \
            'solvent':'imp','color_style':'g--','label':'ASMD 100 tps'})
    #plot_pmf(**{'method':'rsmd','sample_size':100,'velocity':v, \
            #'solvent':'imp','color_style':'c--','label':'FR-ASMD 100 tps'})
    plot_pmf(**{'method':'asmd','sample_size':400,'velocity':v, \
            'solvent':'imp','color_style':'b--','label':'ASMD 400 tps'})
    plot_pmf(**{'method':'asmd','sample_size':800,'velocity':v, \
            'solvent':'imp','color_style':'r--','label':'ASMD 800 tps'})
...
def plot_vel(vel,vel2):
    ...
    plot_pmf(**{'method':'asmd','sample_size':100,'velocity':v, \
            'solvent':'imp','color_style':'g--','label':'ASMD 100 tps'})
    #plot_pmf(**{'method':'asmd','sample_size':100,'velocity':x, \
     #       'solvent':'imp','color_style':'k--','label':'ASMD 100 tps (1 $\AA$/ns) '})
    plot_pmf(**{'method':'asmd','sample_size':400,'velocity':v, \
            'solvent':'imp','color_style':'b--','label':'ASMD 400 tps'})
    plot_pmf(**{'method':'asmd','sample_size':800,'velocity':v, \
            'solvent':'imp','color_style':'r--','label':'ASMD 800 tps'})
...
#plt.xlabel('end-to-end distance ($\AA$)',fontproperties=fpropxxl)
#plt.ylabel('PMF (kcal/mol)',fontproperties=fpropxxl)
fig.text(0.55,0.03,'end-to-end distance ($\AA$)',ha='center',va='center',fontproperties=fpropl,fontname='Times New Roman')
fig.text(0.09,0.5,'PMF (kcal/mol)',ha='center',va='center',fontproperties=fpropl,fontname='Times New Roman',rotation='vertical')
plt.subplots_adjust(left=0.20,right=0.90,top=0.90,bottom=0.10)
# DRAW__________________________________________________
plt.draw()
#plt.show()
#plt.savefig('original.png')
plt.savefig('FR-ASMDimp10and100final.png')
plt.savefig('FR-ASMDimp10and100final.svg')
